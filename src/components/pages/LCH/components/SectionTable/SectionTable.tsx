...
import { type Section, createDiameterLength, type ColumnValue, Parameter } from "@/lib/types"
...
export function SectionTable({ section, onSectionChange }: SectionTableProps) {
  ...
  const handleValueChange = (paramKey: string, column: "normative" | "plan" | "fact", value: string | number) => {
  ...
  const handleSelectChange = (value: string) => {
    const updatedSection = { ...section }
    const param = updatedSection.parameters.find((p) => p.key === "exhaustionMethod")
  ...
  useEffect(() => {
    ...
    const calculateAveragePressure = () => {
      ...
      if (startPressure && endPressure && avgPressure) {
        ...
        const normValue = calculateAvg(startPressure.values.normative.value, endPressure.values.normative.value)
        const planValue = calculateAvg(startPressure.values.plan.value, endPressure.values.plan.value)
        const factValue = calculateAvg(startPressure.values.fact.value, endPressure.values.fact.value)
    ...
    const calculateAverageTemperature = () => {
      ...
      if (startTemp && endTemp && avgTemp) {
        ...
        const normValue = calculateAvg(startTemp.values.normative.value, endTemp.values.normative.value)
        const planValue = calculateAvg(startTemp.values.plan.value, endTemp.values.plan.value)
        const factValue = calculateAvg(startTemp.values.fact.value, endTemp.values.fact.value)
    ...
    const calculateGasExhaustionPercentage = () => {
      ...
      if (pressureBefore && pressureAfter && percentage) {
        ...
        const planValue = calculatePercentage(pressureBefore.values.plan.value, pressureAfter.values.plan.value)
        const factValue = calculatePercentage(pressureBefore.values.fact.value, pressureAfter.values.fact.value)
    ...
    section.parameters.find((p) => p.key === "geometricVolume")?.values.normative.value,
    section.parameters.find((p) => p.key === "geometricVolume")?.values.plan.value,
    section.parameters.find((p) => p.key === "geometricVolume")?.values.fact.value,

    // Зависимости для среднего давления
    section.parameters.find((p) => p.key === "excessPressureBeforeDisconnectStart")?.values.normative.value,
    section.parameters.find((p) => p.key === "excessPressureBeforeDisconnectStart")?.values.plan.value,
    section.parameters.find((p) => p.key === "excessPressureBeforeDisconnectStart")?.values.fact.value,
    section.parameters.find((p) => p.key === "excessPressureBeforeDisconnectEnd")?.values.normative.value,
    section.parameters.find((p) => p.key === "excessPressureBeforeDisconnectEnd")?.values.plan.value,
    section.parameters.find((p) => p.key === "excessPressureBeforeDisconnectEnd")?.values.fact.value,

    // Зависимости для средней температуры
    section.parameters.find((p) => p.key === "temperatureBeforeDisconnectStart")?.values.normative.value,
    section.parameters.find((p) => p.key === "temperatureBeforeDisconnectStart")?.values.plan.value,
    section.parameters.find((p) => p.key === "temperatureBeforeDisconnectStart")?.values.fact.value,
    section.parameters.find((p) => p.key === "temperatureBeforeDisconnectEnd")?.values.normative.value,
    section.parameters.find((p) => p.key === "temperatureBeforeDisconnectEnd")?.values.plan.value,
    section.parameters.find((p) => p.key === "temperatureBeforeDisconnectEnd")?.values.fact.value,

    // Зависимости для процента выработки газа
    section.parameters.find((p) => p.key === "averageAbsolutePressureBeforeDisconnect")?.values.normative.value,
    section.parameters.find((p) => p.key === "averageAbsolutePressureBeforeDisconnect")?.values.plan.value,
    section.parameters.find((p) => p.key === "averageAbsolutePressureBeforeDisconnect")?.values.fact.value,
    section.parameters.find((p) => p.key === "excessPressureAfterExhaustionWithoutMKSStartEditable")?.values.normative
      .value,
    section.parameters.find((p) => p.key === "excessPressureAfterExhaustionWithoutMKSStartEditable")?.values.plan.value,
    section.parameters.find((p) => p.key === "excessPressureAfterExhaustionWithoutMKSStartEditable")?.values.fact.value,
  ])

  // Функция для рендеринга ячейки в зависимости от редактируемости
  const renderCell = (columnValue: ColumnValue, onChange?: (value: string | number) => void) => {
    if (columnValue.editable && onChange) {
      return (
        <TableCell className="border">
          <Input
            ...
            onChange={(e) => {
              const value = typeof columnValue.value === "number" ? Number.parseFloat(e.target.value) : e.target.value
              onChange(value)
            }}
          />
        </TableCell>
      ...
      return <TableCell className="border bg-gray-100">{columnValue.value || ""}</TableCell>
    }
  }
  ...
  const renderParameterRow = (paramKey: string, showName = true) => {
    ...
    return (
      <TableRow>
        {showName && <TableCell className="font-medium border">{param.name}</TableCell>}
        {showName && <TableCell className="border">{param.unit}</TableCell>}
        {renderCell(param.values.normative, (value) => handleValueChange(paramKey, "normative", value))}
        {renderCell(param.values.plan, (value) => handleValueChange(paramKey, "plan", value))}
        {renderCell(param.values.fact, (value) => handleValueChange(paramKey, "fact", value))}
      </TableRow>
    )
  }

  // Функция для рендеринга двойной строки параметра (с редактируемой и нередактируемой частями)
  const renderDoubleParameterRow = (nonEditableKey: string, editableKey: string) => {
    const nonEditableParam = section.parameters.find((p) => p.key === nonEditableKey)
    ...
    return (
      <>
        ...
        <TableRow>
          {renderCell(editableParam.values.normative, (value) => handleValueChange(editableKey, "normative", value))}
          {renderCell(editableParam.values.plan, (value) => handleValueChange(editableKey, "plan", value))}
          {renderCell(editableParam.values.fact, (value) => handleValueChange(editableKey, "fact", value))}
        </TableRow>
      </>
    )
  }

  // Функция для рендеринга группы строк с одинаковым названием, но разными единицами измерения
  const renderMultipleUnitsRow = (baseKey: string, units: string[], showName = true) => {
    const params = units.map((unit) => section.parameters.find((p) => p.key === `${baseKey}${unit}`))
    ...
    return (
      <>
        <TableRow>
          {showName && (
            <TableCell className="font-medium border" rowSpan={units.length}>
              {firstParam.name}
            </TableCell>
          )}
          <TableCell className="border">{firstParam.unit}</TableCell>
          {renderCell(firstParam.values.normative, (value) =>
            handleValueChange(`${baseKey}${units[0]}`, "normative", value),
          )}
          {renderCell(firstParam.values.plan, (value) => handleValueChange(`${baseKey}${units[0]}`, "plan", value))}
          {renderCell(firstParam.values.fact, (value) => handleValueChange(`${baseKey}${units[0]}`, "fact", value))}
        </TableRow>
        {units.slice(1).map((unit, index) => {
          const param = params[index + 1]!
          return (
            <TableRow key={`${baseKey}${unit}`}>
              <TableCell className="border">{param.unit}</TableCell>
              {renderCell(param.values.normative, (value) =>
                handleValueChange(`${baseKey}${unit}`, "normative", value),
              )}
              {renderCell(param.values.plan, (value) => handleValueChange(`${baseKey}${unit}`, "plan", value))}
              {renderCell(param.values.fact, (value) => handleValueChange(`${baseKey}${unit}`, "fact", value))}
            </TableRow>
          )
        })}
      </>
    ...
    <div className="overflow-x-auto">
      <Table className="border">
        ...
        <TableBody>
          ...
          {renderDoubleParameterRow("atmosphericPressure", "atmosphericPressureEditable")}

          {/* 3. Плотность газа */}
          {renderDoubleParameterRow("gasDensity", "gasDensityEditable")}

          {/* 4. Теплота сгорания низшая */}
          {renderDoubleParameterRow("lowerHeatingValue", "lowerHeatingValueEditable")}

          {/* 5-6. Диаметр и длина участков */}
          {section.diameterLengths.map((item, index) => (
            <>
              <TableRow key={`diameter-${index}`}>
                <TableCell className="font-medium border">Диаметр участка L{index + 1}</TableCell>
                <TableCell className="border">мм</TableCell>
                {renderCell(item.diameter.values.normative, (value) =>
                  handleDiameterLengthChange(index, "diameter", "normative", value),
                )}
              ...
              <TableRow key={`length-${index}`}>
                <TableCell className="font-medium border">Длина участка L{index + 1}</TableCell>
                <TableCell className="border">км</TableCell>
                {renderCell(item.length.values.normative, (value) =>
                  handleDiameterLengthChange(index, "length", "normative", value),
                )}
          ...
          <TableRow>
            <TableCell colSpan={5} className="border">
              <Button variant="outline" size="sm" onClick={addDiameterLength} className="w-full">
          ...
          <TableRow>
            <TableCell className="font-medium border">Способ выработки без МКС (ГРС, перепуск....)</TableCell>
            <TableCell className="border">-</TableCell>
            <TableCell className="border" colSpan={3}>
              <Select
                value={
                  (section.parameters.find((p) => p.key === "exhaustionMethod")?.values.normative.value as string) || ""
                }
          ...
          {renderParameterRow("averageAbsolutePressureAfterExhaustionWithoutMKS")}
          ...
          {renderParameterRow("totalExhaustedWithoutMKSNormativeFuelEquivalent", false)}
          ...
          {renderParameterRow("totalExhaustedWithMKSNormativeFuelEquivalent", false)}
          ...
          {renderParameterRow("totalExhaustedWithoutMKSPlanFuelEquivalent", false)}
          ...
          {renderParameterRow("totalExhaustedWithoutMKSFactFuelEquivalent", false)}
          ...
          {renderParameterRow("totalGasForMixtureRemovalNormativeFuelEquivalent", false)}
          ...
          {renderParameterRow("totalGasForMixtureRemovalPlanFuelEquivalent", false)}
          ...
          {renderParameterRow("totalGasForMixtureRemovalFactFuelEquivalent", false)}
          ...
          {renderParameterRow("totalVentedGasLowPressureNormativeFuelEquivalent", false)}
          ...
          {renderParameterRow("totalVentedGasLowPressurePlanFuelEquivalent", false)}
          ...
          {renderParameterRow("totalVentedGasLowPressureFactFuelEquivalent", false)}
          ...
          {renderParameterRow("gasBeforeRepairExcludingTPLowPressureCubicMeters")}
          {renderParameterRow("gasBeforeRepairExcludingTPLowPressureFuelEquivalent", false)}
          ...
          {renderParameterRow("totalGasBeforeRepairExcludingTPLowPressureNormativeCubicMeters")}
          {renderParameterRow("totalGasBeforeRepairExcludingTPLowPressureNormativeFuelEquivalent", false)}
          {renderParameterRow("totalGasBeforeRepairExcludingTPLowPressureNormativeKg", false)}

          {/* 83. ИТОГО Перед проведением ремонтных работ, за исключением объема ТП, при Р<1 МПа, план */}
          {renderParameterRow("totalGasBeforeRepairExcludingTPLowPressurePlanCubicMeters")}
          {renderParameterRow("totalGasBeforeRepairExcludingTPLowPressurePlanFuelEquivalent", false)}
          {renderParameterRow("totalGasBeforeRepairExcludingTPLowPressurePlanKg", false)}

          {/* 84. ИТОГО Перед проведением ремонтных работ, за исключением объема ТП, при Р<1 МПа, факт */}
          {renderParameterRow("totalGasBeforeRepairExcludingTPLowPressureFactCubicMeters")}
          {renderParameterRow("totalGasBeforeRepairExcludingTPLowPressureFactFuelEquivalent", false)}
          {renderParameterRow("totalGasBeforeRepairExcludingTPLowPressureFactKg", false)}
